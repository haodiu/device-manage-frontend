...
const ProductsManagement = () => {
  ...
  React.useEffect(() => {
    ...
    fetch(hostLogbookTypes, {
      ...
      .catch((err) => console.log(err));
  }, [hostUsers, hostLogbookTypes, userDetail.id, userDetail.accessToken]);
  ...
  const getDeviceLogbook = async (id) => {
    ...
    if (!response.ok) {
      throw new Error("Failed to get logbook for user");
    }
    const res = await response.json();
  ...
  const confirmLogbook = async () => {
    const response = await fetch(hostLogbooks + deviceLogbook.logbookId + "/confirm", {
      method: "PUT",
      headers: {
        Authorization: "Bearer " + userDetail.accessToken,
        "Content-Type": "application/json",
    ...
    <>
      <div className="main-panel">
        <div className="content-wrapper">
          ...
          {/* view logbook */}
          <Modal className="modal" show={showDeviceLogbook} onHide={closeDeviceLogbook}>
              ...
              <Form.Group as={Row} className="mp-3">
                ...
                <Form.Group as={Row} className="mp-3">
                  ...
                  <Form.Control
                    type="text"
                    readOnly={true}
                    value={getDate(deviceLogbook.createdAt)}
                  />
                </Form.Group>
                ...
                <Form.Group as={Row} className="mp-3">
                  ...
                  <Form.Control
                    type="text"
                    placeholder={deviceLogbook.confirmedDescription ? deviceLogbook.confirmedDescription : "Chưa được xác nhận khi nhận thiết bị"}
            ...
            <Modal.Footer>
              ...
              {deviceLogbook.confirmed === false && <Button variant="primary" onClick={() => confirmLogbook()}>
                Xác nhận
              </Button>}
            </Modal.Footer>
          </Modal>
